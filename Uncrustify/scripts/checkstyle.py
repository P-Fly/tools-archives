...
def fix_files(files):
	...
	for i in files:
		...
		cur_ret = run("{} -q --replace --no-backup -c {} {} > /dev/null".format(UNCRUSTIFY_BIN, CONFIG_FILE, i))
		if cur_ret is False:
...
def check_files(files):
	...
	for i in files:
		log("Processing {}".format(i))
		cur_ret = run("{} -q -c {} -f {} | diff {} - > /dev/null".format(UNCRUSTIFY_BIN, CONFIG_FILE, i, i))
		if cur_ret is False:
			print("File fails format check: {}".format(i))
			cur_ret = run("{} -c {} -f {} 2> /dev/null | diff -u --tabsize=4 {} -".format(UNCRUSTIFY_BIN, CONFIG_FILE, i, i))
			print("")
		ret = ret and cur_ret
	return ret
